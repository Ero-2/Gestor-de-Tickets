...
switch ($method) {
    case 'GET':
        ...
        try {
            ...
            if ($departamento_id !== null && is_numeric($departamento_id)) {
                $where_clauses[] = 'u."IdDepartamentos" = :departamento_id';
                $params['departamento_id'] = (int)$departamento_id;
            }

            // Filtro por tipo de usuario (1 = admin, 2 = usuario)
            $tipo_usuario = isset($_GET['tipo_usuario']) ? trim($_GET['tipo_usuario']) : null;
            if ($tipo_usuario !== null && in_array($tipo_usuario, ['1', '2'])) {
                $where_clauses[] = 'u."IdTipoDeUsuario" = :tipo_usuario';
    ...
    // --- El resto del código (POST, PUT, DELETE) queda exactamente IGUAL ---
    case 'POST':
        // Crear nuevo usuario
        ...
        $id_tipo_usuario = intval($data['id_tipo_usuario']) ?? 2;
    ...
    case 'PUT':
        ...
        $email = $data['email'] ?? null;
        $puesto = $data['puesto'] ?? null;
        $id_departamentos = $data['id_departamentos'] ?? null;
        $id_tipo_usuario = $data['id_tipo_usuario'] ?? null;

        if (!$id_usuario || (!$nombre && !$email && !$puesto && $id_departamentos === null && $id_tipo_usuario === null)) {
            http_response_code(400);
            echo json_encode(['error' => 'Se requiere ID de usuario y al menos un campo para actualizar.']);
            exit;
        }
        ...
        try {
            ...
            if ($id_tipo_usuario !== null) {
                ...
                if ($id_tipo_usuario < 1 || $id_tipo_usuario > 2) {
                    http_response_code(400);
